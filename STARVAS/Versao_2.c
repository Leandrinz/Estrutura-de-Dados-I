...
int comparar_scores(const void *a, const void *b) {
    ...
    if (scoreA->tempo < scoreB->tempo) return 1;    // Tempo B é maior (deve vir primeiro)
    if (scoreA->tempo > scoreB->tempo) return -1;   // Tempo A é maior (deve vir primeiro)
    return 0; // Tempos iguais
}

// Função que você deve ter definido:
// int buscar_jogador_linear(const Score *scores, int num_scores, const char *nome_alvo);

int buscar_jogador_linear(const Score *scores, int num_scores, const char *nome_alvo) {
    // A busca linear percorre cada elemento do array, do início ao fim (i=0 até num_scores-1).
    for (int i = 0; i < num_scores; i++) {
        // Usa strcmp para comparar strings. Retorna 0 se forem iguais.
        if (strcmp(scores[i].nome, nome_alvo) == 0) {
            // Se encontrou o jogador, retorna a posição (índice) dele.
            return i; 
        }
    }
    // Se o loop terminar sem encontrar o nome, retorna -1.
    return -1;
}
...
void salvar_e_ordenar_historico(const char *novo_nome, double novo_tempo) {
    ...
    Score scores[1000]; 
    int num_scores = 0;
    int indice_de_escrita; // Variável para rastrear onde colocar o novo score
    
    // --- 1. LEITURA DOS SCORES EXISTENTES ---
    // (O bloco de leitura permanece o mesmo)
    FILE *leitura_file = fopen("historico.txt", "r");
    
    if (leitura_file != NULL) {
        // Tenta ler os scores do arquivo
    ...
    // --------------------------------------------------------------------
    // --- 2. BUSCA LINEAR E ADIÇÃO/ATUALIZAÇÃO ---
    // --------------------------------------------------------------------
    
    // Procura se o nome do jogador já está na lista lida:
    int indice_existente = buscar_jogador_linear(scores, num_scores, novo_nome);
    
    if (indice_existente != -1) {
        // A) JOGADOR ENCONTRADO (ATUALIZAÇÃO)
        
        // Se o novo tempo for maior (melhor), atualiza o score.
        if (novo_tempo > scores[indice_existente].tempo) {
            ...
            // Se não é um recorde, não precisa reordenar e reescrever. Podemos sair.
            return;
        }
        
        // O índice de escrita é o índice que já existe
        indice_de_escrita = indice_existente; 
        
    } else {
        // B) JOGADOR NOVO (ADIÇÃO)
        if (num_scores < 1000) {
            // O índice de escrita é a próxima posição disponível
            indice_de_escrita = num_scores; 
            
            strcpy(scores[indice_de_escrita].nome, novo_nome);
            scores[indice_de_escrita].tempo = novo_tempo;
            num_scores++; // Aumenta a contagem de scores, pois adicionamos um novo
        }
    }

    // --- 3. ORDENAÇÃO E REESCRITA ---
    // (A ordenação deve ser feita após a atualização/adição para que o score fique na posição correta)
    
    qsort(scores, num_scores, sizeof(Score), comparar_scores);
    
    // ... (o bloco de reescrita permanece o mesmo)
    FILE *escrita_file = fopen("historico.txt", "w"); 
