...
void alterar(Lista *plista, int mat, char nome[]){
    ...
    if (pi == NULL){
        printf("Estudante nao pode ser encontrado\n");
    }
    else{
...
void ordenar(Lista *plista){
    ...
    for (pi = plista->inicio; pi->prox != NULL; pi->prox){
        for (pj = plista->inicio; pi->prox != pfim; pj->prox){
